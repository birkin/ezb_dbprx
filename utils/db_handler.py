...
class DB_Handler(object):
  ...
  def execute_sql(self, sql):
    ...
    try:
      ...
      # self.file_logger.debug( u'in dev_code.db_handler.execute_sql(); dict_list: %s' % unicode(repr(dict_list)) )
      dict_list = self._unicodify_resultset( dict_list )
      # self.file_logger.debug( u'in dev_code.db_handler.execute_sql(); dict_list NOW: %s' % unicode(repr(dict_list)) )
      return dict_list
  ...
  def _setup_db_connection( self ):
    ...
    try:
      ...
      message = u'in dev_code.db_handler._setup_db_connection(); error: %s' % unicode( repr(e).decode(u'utf8', u'replace') )
      self.file_logger.error( message )
  ...
  def _close_db_connection( self ):
    ...
    try:
      ...
      # self.file_logger.debug( u'in dev_code.db_handler._close_db_connection(); db connection closed' )
      return
    except Exception as e:
